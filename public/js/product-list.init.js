            ...
            data: (function (row) {
                return gridjs.html('<div class="text-center dropdown">\
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">\
                        <li>\
                            <a class="dropdown-item edit-item" data-id="'+ row.id + '" href="product-create.html"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a>\
                        </li>\
                        <li>\
                            <a class="dropdown-item remove-list" data-id="'+ row.id +'" data-bs-toggle="modal" href="#removeItemModal"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i>Delete</a>\
                        </li>\
                    </ul>\
                </div>');
            })
...
function listProd(productList){
      ...
      //When the add product button is clicked, the edit input value should be null
    ...
    const getItem = async function () {
        try {
            ...
            const response = await fetch(`http://127.0.0.1:4000/items/back`, headers);
            if (response.ok) {
                const data = await response.json();
                if (document.getElementById("product-list")) {
                    
                    const productList = new gridjs.Grid({
                        columns: columns,
                        sort: true,
                        pagination: {
                            limit: 10
                        },
                        data: data, // Pass the fetched data directly
                       
                    }).render(document.getElementById("product-list"));
                    //Pass the product list object to the function that will perform other operations on it.
...
//When the edit button is clicked, then the id of the item that has the button should be logged to the console, and also set
      //as a session item so it can be used in the create product page to update the product.
      function editProductList(elem) {
        ...
        productListData = productListData.map(function (item) {
            if (item.id == getEditid) {
              ...
              //   console.log(getEditid,item,sessionStorage.getItem('editInputValue'));
            }
            return item;
        });
    };
