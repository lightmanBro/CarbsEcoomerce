...
if (token && username) {
  ...
  document
    ...
    .forEach((div) => (div.innerHTML += username.split("@")[0]));
  console.log(document.querySelectorAll(".username"));
}

var url = "http://127.0.0.1:2500/brands";
...
const newBrand = async function (jsonurl, formData) {
  try {
    const headers = {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      getBrand(url, function (err, data) {
        if (err !== null) {
...
var getBrand = async function (jsonurl, callback) {
  try {
    const brands = await fetch(jsonurl, data);
    if (brands.status === 200) {
      const data = await brands.json();
      callback(null, data);
    } else {
      callback(brands.status, await brands.json());
    }
  } catch (error) {
...
async function loadBrandListData(datas, page) {
  ...
  for (
    ...
    // const logo = await downloadPicture(datas[i].companyLogo)

    document.getElementById("brand-list").innerHTML += `<div class="col">\
        <div class="card brand-widget card-animate">\
            <div class="card-body text-center pb-2">\
                <img src="
      ${await downloadPicture(datas[i].companyLogo)}" alt="" class="brand-img">\
            </div>\
            <div class="card-footer text-center border-0">\
                <h6 class="fs-17">${datas[i].brandName}</h6>\
                <p class="mb-0"><a href="#!" class="link-success stretched-link">${
                  datas[i].brandProducts > 0
                    ? `${datas[i].brandProducts} Items`
                    : "No Items yet"
                }</a></p>\
            </div>\
        </div>\
...
// brandLogo image
document
  .querySelector("#brandLogo-image-input")
...
Array.prototype.slice.call(forms).forEach(function (form) {
  form.addEventListener("submit", function (event) {
    ...
    if (
      ...
      formData.append("brandName", brandNameVal.value);
      formData.append(
        "files",
        document.querySelector("#brandLogo-image-input").files[0]
      );

      //   console.log(document.querySelector("#brandLogo-image-input").files[0],[...formData.entries()]); // Log form data content
      //Create a new Brand
      newBrand(url, formData);
      sortElementsById();
    }
